...
export class PaytrService {
    ...
    constructor() {
        this.config = {
            ...
            merchant_key: process.env.PAYTR_MERCHANT_KEY || process.env.PAYTR_API_KEY || "",
    ...
    async getIframeToken(params: PaytrTokenParams): Promise<string> {
        ...
        const merchant_fail_url = `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/payment/error`;
    ...
    async getIframeTokenWithIp(params: PaytrTokenParams, clientIp: string): Promise<{ token: string; iframeUrl: string }> {
        const {
            merchant_oid,
            email,
            payment_amount,
        ...
        const merchant_ok_url = `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/payment/success?oid=${merchant_oid}`;
        const merchant_fail_url = `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/payment/error?oid=${merchant_oid}`;
        ...
        if (!this.config.merchant_key) {
            throw {
                message: "PAYTR_MERCHANT_KEY (veya PAYTR_API_KEY) eksik! Lütfen .env.local dosyasını kontrol edin.",
                debug: { env_keys: Object.keys(process.env).filter(k => k.startsWith('PAYTR')) }
            };
        }
        ...
        try {
            ...
            if (data.status === "success") {
                return { token: data.token, iframeUrl: "https://www.paytr.com/odeme/guvenli/" + data.token };
            } else {
     ...
     */
    validateCallback(params: { merchant_oid: string; status: string; total_amount: string; hash: string }): boolean {
        const { merchant_oid, status, total_amount, hash } = params;

        const hashStr = merchant_oid + this.config.merchant_salt + status + total_amount;
