...
interface CartContextType {
  cartItems: CartItem[];
  addToCart: (product: Product & { categories?: { name: string } }, quantity?: number) => boolean;
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (savedCart) {
        ...
        const migratedCart = cartData.map((item: CartItem & { categoryId?: number }) => ({
          ...item,
  ...
  const addToCart = (product: Product & { categories?: { name: string } }, quantity: number = 1): boolean => {
    // Cihaz kontrolü - sepette sadece bir cihaz olabilir
    if (isDevice(product.category_id)) {
      const existingDevice = cartItems.find(item => isDevice(item.categoryId));
      if (existingDevice && existingDevice.id !== product.id) {
        setToast({
          message: 'Sepetinizde zaten bir cihaz var. Sepetteki cihazı silip yeni cihaz ekleyebilirsiniz.',
          type: 'warning'
        });
        return false;
      }
    ...
    if (existingItem) {
      ...
      if (isDevice(product.category_id)) {
        setToast({
          message: 'Bu cihaz zaten sepetinizde. Cihazlardan sadece bir adet sipariş verebilirsiniz.',
          type: 'warning'
        });
        return false;
      }
  ...
  const checkout = async (): Promise<boolean> => {
    try {
      ...
      // Başarılı sipariş
      alert(`Siparişiniz başarıyla alındı! Sipariş numaranız: ${data.id}`);
      clearCart();
      setIsCartOpen(false);
      return true;
